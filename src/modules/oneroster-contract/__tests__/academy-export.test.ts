...
test("checked-in shared OneRoster fixture is generated from the Academy export path", async () => {
  ...
  const fixtureDir = path.join(process.cwd(), "fixtures/oneroster/churchcore-academy-rostering-v2");
  const metadata = JSON.parse(readFileSync(path.join(fixtureDir, "fixture.json"), "utf8")) as {
    name: string;
    files: Array<{ filename: string }>;
  };

  assert.equal(metadata.name, "churchcore-academy-rostering-v2");
  assert.deepEqual(
...
function peopleConfiguration(): PeopleConfiguration {
  return {
    institutionProfile: {
      ...
      capabilities: {
        ministryFormation: false, denominationTracking: false, alumniGiving: false, competencyNarrativeGrading: false, academicStandingAutomation: false,
        studentPwa: true,
...
test("export rejects non-admin roles before reading repositories", async () => {
  for (const role of ["registrar", "student", "faculty"] as const) {
    await assert.rejects(buildAcademyOneRosterExportDataset({ actor: { ...actor, roles: [role] }, peopleRepository: { async fetchPeopleConfiguration() { assert.fail("repository reached"); } } }), /Forbidden/);
  }
});

test("export rejects contaminated tenant data and excludes phone information", () => {
  const people = peopleConfiguration();
  const catalog = courseCatalogConfiguration();
  const output = mapAcademyOneRosterDataset(tenantId, people, catalog, registrations());
  assert.equal(output.users.some(user => user.phone !== undefined), false);
  people.people[0].tenantId = "another-tenant";
  assert.throws(() => mapAcademyOneRosterDataset(tenantId, people, catalog, registrations()), /Forbidden cross-tenant/);
});

test("export binds section selection to the tenant and rejects unknown sections", async () => {
  const input = { actor, peopleRepository: fakePeopleRepository(), courseCatalogRepository: fakeCourseCatalogRepository(), registrationRepository: fakeRegistrationRepository() };
  await assert.rejects(buildAcademyOneRosterExportDataset({ ...input, sectionId: "other-section" }), /not found/);
