...
export async function buildAcademyOneRosterExportPackage(
  ...
  assertInstitutionConfigAccess(input.actor, tenantId, "read");
...
export async function buildAcademyOneRosterExportDataset(
  ...
  const [people, catalog, registrations] = await Promise.all([
    peopleRepository.fetchPeopleConfiguration(tenantId),
    courseCatalogRepository.fetchCourseCatalogConfiguration(tenantId),
    registrationRepository.listSectionRegistrations(tenantId),
  ]);
