...
function assertStudentOwnRegistration(actor: AcademyActor, studentPersonId: string) {
  if (actor.roles.includes("student") && !canBypassEnrollmentWindow(actor) && actor.userId !== studentPersonId) {
    throw new AcademyAuthorizationError("Forbidden: students can only register themselves.");
  }
}

function requireOverrideReason(reason: string | undefined) {
  const trimmed = reason?.trim();
  if (!trimmed) {
    throw new Error("overrideReason is required for registrar override.");
  }
  if (trimmed.length > 500) {
    throw new Error("overrideReason must be 500 characters or fewer.");
  }
  return trimmed;
}
...
async function emitOverrideAuditEvent(
  actor: AcademyActor,
  action: "course_registration.override_add" | "course_registration.override_drop",
...
async function runInTransaction<T>(db: Queryable, operation: () => Promise<T>): Promise<T> {
  if (db.transactionManaged) {
    return operation();
  }
...
export async function registerStudentForSection(
  actor: AcademyActor,
  input: { sectionId: string; studentPersonId: string; overrideReason?: string },
  db: Queryable,
): Promise<SectionRegistration> {
  assertTenantIsolation(actor, actor.tenantId);
  assertStudentOwnRegistration(actor, input.studentPersonId);

  return runInTransaction(db, async () => {
    // Lock the section row before duplicate and capacity checks so concurrent adds
    // for the same section serialize around the count and insert.
    ...
    if (!section.enrollment_window_id) {
      ...
      overrideReason = requireOverrideReason(input.overrideReason);
    }
    ...
    const mappedRegistration = mapRegistration(registration.rows[0]);
    if (overrideReason) {
      await emitOverrideAuditEvent(
        actor,
        "course_registration.override_add",
        mappedRegistration.id,
        overrideReason,
        db,
      );
    }
...
export async function dropStudentFromSection(
  actor: AcademyActor,
  input: { registrationId: string; overrideReason?: string },
  db: Queryable,
): Promise<void> {
  assertTenantIsolation(actor, actor.tenantId);
  ...
  } else if (!registration.enrollment_window_id) {
    overrideReason = requireOverrideReason(input.overrideReason);
  }
  ...
  if (overrideReason) {
    await emitOverrideAuditEvent(
      actor,
      "course_registration.override_drop",
