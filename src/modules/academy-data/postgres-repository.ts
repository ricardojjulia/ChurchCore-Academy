...
export class AcademyDataRepository {
  async loadDataset(): Promise<AcademyDataset> {
    ...
    const [
      ...
      pool.query("select * from academy_institution_profiles limit 1"),
      pool.query("select * from academy_calendar_profiles limit 1"),
      pool.query("select * from academy_academic_years order by starts_on asc"),
      pool.query("select * from academy_academic_periods order by sequence asc, starts_on asc"),
      pool.query("select * from academy_enrollment_windows order by opens_at asc"),
      pool.query("select * from academy_grading_windows order by opens_at asc"),
      pool.query("select * from academy_transcript_periods order by posting_opens_at asc"),
      pool.query("select * from academy_institution_subdivisions order by subdivision_type asc, name asc"),
      pool.query("select * from academy_course_catalog_profiles limit 1"),
      pool.query("select * from academy_courses order by code asc"),
      ...
      pool.query("select * from academy_staff_profiles order by staff_number asc"),
      pool.query("select * from academy_student_relationships order by student_person_id asc, relationship_type asc"),
      pool.query("select * from academy_account_links order by provider asc, external_subject asc"),
