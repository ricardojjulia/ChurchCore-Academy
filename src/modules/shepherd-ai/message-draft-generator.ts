...
export class MessageDraftGenerator {
  draft(workflowCode: WorkflowCode, context: AcademyContext): string | undefined {
    switch (workflowCode) {
      ...
      case "calendar_setup_review":
        return `Hello,\n\nChurchCore Academy has detected one or more configuration gaps in the academic calendar for ${context.entityLabel}. Please review the calendar validation report and complete the remaining setup items, including academic year dates, academic period alignment, enrollment windows, and grading windows.\n\nThank you,\n${context.ownerRole}`;
      default:
        return undefined;
    }
  }
