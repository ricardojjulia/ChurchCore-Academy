...
| [#110](https://github.com/ricardojjulia/ChurchCore-Academy/pull/110) | docs: add 2026-09-15 daily checkup report | Safe (docs-only) | **Closed without merge** — its branch was an ancestor of the branches used for #111 and #112, so its content (the original report file) landed on `main` via #111's squash-merge instead. No content was lost; this was a branch-cleanup side effect (deleting #110's head branch while investigating its review comments auto-closed the PR before it was separately merged) — flagged here for transparency rather than quietly leaving the PR list inaccurate. |
| [#111](https://github.com/ricardojjulia/ChurchCore-Academy/pull/111) | fix: exclude draft ministry-formation evaluations from student view | Safe (isolated data-filter fix, fully tested, no schema/auth/tenant-isolation change) | **Merged** to `main` after CI passed. Found via automated PR review on #110 — see §2.5. Note: this squash-merge's diff on `main` also carries the original report-file commit from #110's branch (see #110's row) — an artifact of branch ancestry, not scope creep in the fix itself. |
| [#112](https://github.com/ricardojjulia/ChurchCore-Academy/pull/112) | docs: correct report PR-title accuracy and add the evaluation-privacy fix | Safe (docs-only) | **Merged** to `main` after CI passed |

No PR was left in "needs review" status this run — nothing touched schema, auth logic, tenant isolation, or added a dependency. One git-hygiene slip: PR #110 was closed unmerged by an over-eager branch-cleanup step (deleting a PR's head branch closes the PR) after its content had already been carried forward by branch ancestry — worth being more careful about which branches are "done" before deleting them.
