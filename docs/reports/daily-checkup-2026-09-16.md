...
- **Fixed:** three admin pages shipped in yesterday's "Surface the Built Differentiators" factory work (`/admin/denomination`, `/admin/alumni`, `/admin/settings/grading`) had no sidebar nav entry at all — reachable only by typing the exact URL. Added nav entries, correctly gated by both institution capability AND the destination page's own role allowlist.
