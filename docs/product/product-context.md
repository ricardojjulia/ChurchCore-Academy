...
The full Core Academic Loop (steps 1–11 above) is implemented, with real Postgres-backed modules, admin UI, and tests for every step, and — as of 2026-09-17 — verified end-to-end in the live app for the first time in this codebase's history, including the final grade-to-transcript handoff (see the update below the table):
...
**Verification walkthrough completed 2026-09-16; grading and transcript gaps closed 2026-09-17.** The complete loop was first walked in one browser session on 2026-09-16 — see `docs/reports/walkthrough-2026-09-16.md`. It found and fixed five real defects along the way (academic period 404s, silent course-form validation failure, a wrong gradebook nav link, a missing assignment-creation page, and a missing `assignment_type` column wiring), plus two deeper issues, both now closed:

1. The official-grade-posting path (`submitGradeAction`, and the older `gradeSubmission` REST endpoint) had a broken database trigger blocking every write since 2026-06-26, invisible from the UI alone. Fixed in [PR #126](https://github.com/ricardojjulia/ChurchCore-Academy/pull/126) (merged), with the same resubmission/consistency-check pattern applied to both writers in [PR #131](https://github.com/ricardojjulia/ChurchCore-Academy/pull/131) and [PR #132](https://github.com/ricardojjulia/ChurchCore-Academy/pull/132) (both merged).
2. `submitDraftFinalGrade` — the function that turns per-assignment grades into a section's official final grade, feeding step 11 ("Produce a Transcript Entry") — had zero callers anywhere in the codebase and had never successfully run (it referenced schema columns that don't exist). Fixed, wired into a new faculty UI, and two RLS gaps it exposed were closed, in [PR #133](https://github.com/ricardojjulia/ChurchCore-Academy/pull/133) (merged).

**Steps 10–11 are now confirmed working end-to-end on `main`**, verified live: a faculty member submits a final grade → the registration is marked complete → the student appears as a transcript-entry candidate on the registrar's page → posting produces a correct, immutable transcript entry. This is the first time the full grade-to-transcript pipeline has worked end-to-end in this codebase's history. The computed-grades page and section draft→open UI merged separately and cleanly in [PR #127](https://github.com/ricardojjulia/ChurchCore-Academy/pull/127). A 2026-09-17 checkup also fixed an unrelated role-gating nav bug ([PR #128](https://github.com/ricardojjulia/ChurchCore-Academy/pull/128), merged).
