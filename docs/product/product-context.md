...
**Verification walkthrough completed 2026-09-16; one fix still pending merge.** The complete 12-step chain was walked in one browser session for the first time — see `docs/reports/walkthrough-2026-09-16.md`. It found and fixed five real defects along the way (academic period 404s, silent course-form validation failure, a wrong gradebook nav link, a missing assignment-creation page, and a missing `assignment_type` column wiring — all merged), plus a deeper issue: the grading pipeline's official-grade-posting path (`submitGradeAction`) has a broken database trigger blocking every write since 2026-06-26, invisible from the UI alone. As a result, step 10 ("Record a Grade for an Enrollment") only partially works today: advisory/assignment-level grade entry works, but promoting a grade to an official record does not. A fix for that trigger, plus the previously-missing "Submit for Posting" and computed-grades UI, was built and verified end-to-end against a local branch in [PR #126](https://github.com/ricardojjulia/ChurchCore-Academy/pull/126) — **but as of 2026-09-17 that PR is still open, unreviewed, and unmerged, so the trigger bug is still present on `main` today.** Do not claim steps 10–11 work end-to-end until #126 merges and a re-walk confirms it on `main`. The computed-grades page and section draft→open UI from the same effort merged separately and cleanly in [PR #127](https://github.com/ricardojjulia/ChurchCore-Academy/pull/127) — those two are live. A 2026-09-17 checkup also fixed an unrelated role-gating nav bug (ShepherdAI Queue link shown to staff roles that couldn't access the page, [PR #128](https://github.com/ricardojjulia/ChurchCore-Academy/pull/128), merged).
