...
5. Is gated by a new institution capability flag (`aiWordingAssistance`), following the exact pattern already established for `shepherdAiRecommendations` and the other 10 capability flags in `InstitutionCapabilitySet` — an institution that hasn't enabled it gets Ghost Mode (HTTP 451), same as any other disabled capability.
6. Logs every call to the existing immutable audit-event pattern: task type, model used, anonymized-token count, cost, and outcome (approved-as-is / edited / discarded) — **not** the raw prompt or raw completion text, to avoid retaining PII-adjacent content in audit storage longer than necessary.
