...
| `warning_notice_draft` | a persisted `academy_standing_evaluations` row (append-only, added in PR #121; the in-memory `StudentAcademicStandingEvaluation` has no ID to load by) | standing type (`warning`/`probation`) from `computed_standing_types`, a triggering rule category derived from the stored summary via a fixed mapping (not full rule text), GPA band from a fixed band table | student name/ID, GPA exact value (use a band: "below 2.0"), financial info |
...
**3. An independent second layer that doesn't trust the first.** This repo already has a fail-closed precedent for a related problem: `validateAuditMetadata()` in `src/modules/audit/postgres-repository.ts` rejects audit metadata whose **keys** match a secret-shaped pattern before persistence. That checks key names; this needs a sibling that checks **value content** — a scanner run over the *final constructed payload* immediately before the OpenRouter call, with no knowledge of which allow-list produced it. Pattern matching covers what patterns can catch (email, phone, SSN/government-ID, account/card numbers, street-address shapes); names and free text can't be reliably caught by regex, so the scanner also checks the payload against the source record's own identifying values (the student's name, email, phone, address, IDs, guardian contact) by exact, case-insensitive search. Even so, the scanner is defense-in-depth behind the typed allow-list — the guarantee rests on the allow-list, and the scanner exists to catch allow-list bugs, not to replace it. If it fires: reject the request (never redact-and-send), log it as a security event, and auto-disable `aiWordingAssistance` for that tenant pending investigation. A bug in the allow-list and a bug in the scanner would both have to fail identically to leak anything — that's what "independent" buys.
...
- `AiGatewayUsageRecord`: `tenantId`, `taskType`, `model`, `promptTokens`, `completionTokens`, `costUsd` (computed from OpenRouter's per-request cost metadata, not estimated), `draftRequestId` (an opaque ID minted before the provider call), `createdAt`. One row per completed request, enforced by a unique `(tenant_id, draft_request_id)` constraint: a retry after the provider call succeeded reuses the same `draftRequestId`, so the insert is idempotent and reconciliation isn't skewed by duplicate rows.
...
- `src/modules/ai-gateway/service.ts` — `requestDraft(actor, taskType, sourceRecordId)`, in three phases:
  1. **Authorize and load, in one short database context.** Assert the `aiWordingAssistance` capability, then apply the *originating surface's* record-level authorization before loading, not just the tenant-wide flag. For example, `advising_followup_draft` from `/faculty/shepherd` must pass the same section-ownership filter that page applies, so an in-tenant caller can't draft for a student they couldn't already see. Load the source through the existing tenant-scoped repository (`shepherd-ai`/`grading-records`/`admissions`, with no duplicated queries), anonymize, mint the `draftRequestId`, then close the context.
  2. **Call OpenRouter outside any transaction.** `withCapabilityContext` → `withAcademyDatabaseContext` holds a transaction and pool connection for the handler's lifetime; holding it across an unbounded third-party call contradicts ADR 0019.
  3. **Persist usage in a separate, idempotent transaction** keyed on `draftRequestId`, then rehydrate and return `{ draftRequestId, draft }`.
- `recordDraftDecision(actor, draftRequestId, outcome)` with `outcome: "approved" | "edited" | "discarded"`: writes the audit outcome ADR 0070 requires, since the decision happens after `requestDraft` has returned. It stores the reference and outcome only, never draft text.
- API routes under `src/app/api/academy/ai-gateway/draft/route.ts` (POST), `src/app/api/academy/ai-gateway/draft/[draftRequestId]/decision/route.ts` (POST, records approve/edit/discard), and `src/app/api/academy/ai-gateway/consumption/route.ts` (GET, admin-only).
...
- A review modal: shows the draft, an explicit "This draft was AI-generated from anonymized data — review before use" notice, an editable text area, and Approve/Discard actions, each of which calls the decision endpoint (approve is recorded as `edited` if the text was changed). Approving copies the (rehydrated, human-editable) text into the existing communications-compose flow — it does not send anything itself.
...
- A test for the independent content scanner: feed it payloads with pattern-shaped values (email, phone, SSN/government ID, account numbers, addresses) and with the source record's own name and contact values that the allow-list logic would have missed, and assert it rejects them (fail-closed) rather than silently stripping and continuing.
