...
| Alumni & giving | `src/modules/people/alumni.ts`, full test coverage, API routes live | Admin UI, directory, campaign layer — still missing as of 2026-09-15; a technical brief has been written and approved, build in progress. Note: campaign layer will ship scoped down to filtering by the existing free-text `fund_designation` field — there is no first-class campaign entity in the schema, and building one is out of scope for this pass. |
